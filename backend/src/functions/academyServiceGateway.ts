  ...
  try{
    ...
    if(serviceType==="messaging"){
      if(operation==="publish"&&request.method==="POST"){
        ...
        const charged=await consume(item,1,"messaging.publish",request,{topic});
      ...
      if(operation==="receive"&&request.method==="GET"){
        ...
        const charged=await consume(item,Math.max(1,result.rowCount||0),"messaging.receive",request,{topic});
      ...
      if(operation==="ack"&&request.method==="POST"){
        ...
        const charged=await consume(item,1,"messaging.ack",request,{id});
    ...
    if(serviceType==="monitoring"&&operation==="events"){
      if(request.method==="POST"){
        ...
        const charged=await consume(item,Math.max(1,bytes/1_048_576),"monitoring.ingest",request,{bytes});
      ...
      const charged=await consume(item,1,"monitoring.query",request);
    ...
    if(["container_compute","machine_learning","functions","document_intelligence","speech_vision"].includes(serviceType)){
      if(operation==="jobs"&&request.method==="POST"){
        ...
        const charged=await consume(item,quantity,`${serviceType}.submit`,request);
      ...
      if(operation==="jobs"&&request.method==="GET"){
        ...
        const charged=await consume(item,1,`${serviceType}.list`,request);
